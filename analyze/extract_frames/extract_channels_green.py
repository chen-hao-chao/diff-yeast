...
def process_one(input_dir: str, output_dir: str, name: str, method: str, expected_frames: int = 193, number: int = 0):
    ...
    for i, frame in enumerate(ImageSequence.Iterator(im)):
        # Convert to RGB to avoid palette/LA modes; then take green channel.
        green = frame.convert("RGB").getchannel("G")  # single-channel (L)
        out_name = f"{name}_{method}_0_0_{i:03d}.tiff"
        out_path = os.path.join(output_dir, out_name)
        try:
            green.save(out_path, format="TIFF", compression="tiff_deflate")
        except Exception as e:
...
def main():
    ap = argparse.ArgumentParser(description="Extract green channel from GIF frames to TIFFs.")
